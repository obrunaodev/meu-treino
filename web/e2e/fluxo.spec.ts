...
test.describe('jornada completa', () => {
  ...
  test('inicia a sessão e conclui o exercício', async () => {
    ...
    await page.getByRole('button', { name: /iniciar intervalo após a série 1/i }).click()
    await expect(page.getByRole('button', { name: /encerrar intervalo após a série 1/i })).toBeVisible()
    await page.reload()
    await expect(page.getByRole('button', { name: /encerrar intervalo após a série 1/i })).toBeVisible()
    await page.getByRole('button', { name: /encerrar intervalo após a série 1/i }).click()
